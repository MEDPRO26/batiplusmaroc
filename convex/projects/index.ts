...
export const initializeDraft = mutation({ args: {}, returns: v.object({ projectId: v.id("projects"), resumed: v.boolean() }), handler: async (ctx) => { const { userId } = await requireClientUser(ctx); const existing = await draftFor(ctx, userId); if (existing) return { projectId: existing._id, resumed: true }; const now = Date.now(); const projectId = await ctx.db.insert("projects", { clientId: userId, countryCode: "MA", surfaceUnknown: false, budgetUnknown: false, visibility: "marketplace", status: "draft", lastCompletedStep: 0, createdAt: now, updatedAt: now }); return { projectId, resumed: false }; } });
