...
export async function Projects() {
  ...
  const tItems = await getTranslations("content.projects");
      ...
      <div className="mx-auto max-w-[1280px] px-[18px] sm:px-6 lg:px-8">
        <header className="mb-10 flex flex-col items-start justify-between gap-7 border-b border-slate-200 pb-9 md:mb-14 md:flex-row md:items-end lg:mb-16">
          <div className="max-w-[900px]">
            <div className="mb-5 flex items-center gap-3">
              ...
              <p className="text-xs font-semibold tracking-[0.16em] text-[#12345a] uppercase">{t("eyebrow")}</p>
            </div>
            <h2
              id="projects-title"
        ...
        <div className="grid gap-5 md:grid-cols-2 md:gap-6 lg:grid-cols-12 lg:gap-7">
          {projects.map((project, index) => {
            ...
            const title = tItems(`${project.slug}.title`);
            const subtitle = tItems(`${project.slug}.subtitle`);
            ...
            if (isFeatured) {
              return (
                <article
                  ...
                  key={project.id}
                >
                  <Link className="absolute inset-0 z-10" href={projectPath(project.slug)} aria-label={t("openAria", { title })} />
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-8 lg:p-10">
                    <div className="mb-5 flex items-center gap-3">
                      ...
                      <span className="text-xs font-medium tracking-[0.12em] text-white/65 uppercase">{t("featured")}</span>
                    </div>
                    <h3 className="max-w-[560px] text-3xl leading-[1.05] font-semibold tracking-[-0.035em] text-white! sm:text-4xl lg:text-5xl">
                      {title}
                    </h3>
                    <p className="mt-4 max-w-[560px] text-sm leading-relaxed text-white/80 sm:text-base">{subtitle}</p>
                  </div>
                </article>
              );
            }
            ...
            return (
              ...
              >
                ...
                <div className="grid grid-cols-[auto_1fr] gap-x-5 p-5 sm:p-6 lg:p-7">
                  ...
                  <div>
                    <h3 className="text-xl leading-tight font-semibold tracking-[-0.025em] text-[#15191f] sm:text-2xl">{title}</h3>
