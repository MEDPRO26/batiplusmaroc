...
export async function Services() {
  ...
  const tItems = await getTranslations("content.services");
    ...
    <section className="bg-[#f8fbfd] py-16 sm:py-20 lg:py-28" aria-labelledby="services-title">
      <div className="mx-auto w-full max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <header className="mx-auto max-w-[930px] text-center">
          <p className="mb-4 text-xs font-bold tracking-[0.16em] text-brand uppercase sm:text-sm">{t("eyebrow")}</p>
          ...
          <p className="mx-auto mt-6 max-w-[760px] text-base leading-relaxed text-muted sm:text-lg">{t("description")}</p>
        </header>
          ...
          {services.map((service) => (
            <Fragment key={service.id}>
              <article className="flex min-h-[360px] flex-col rounded-[28px] border border-slate-200/80 bg-white p-7 shadow-[0_10px_30px_rgba(15,23,42,0.04)] sm:min-h-[380px] md:p-8 lg:p-9">
                ...
                <h3 className="mt-6 text-2xl leading-tight font-semibold tracking-[-0.035em] text-ink! md:text-[30px]">{tItems(`${service.id}.title`)}</h3>
                <p className="mt-5 text-base leading-8 text-muted">{tItems(`${service.id}.description`)}</p>
              ...
              <figure className="group relative min-h-[360px] overflow-hidden rounded-[28px] bg-slate-200 sm:min-h-[380px]">
                ...
                <figcaption className="absolute bottom-6 left-6 text-sm font-medium tracking-wide text-white/90!">{tItems(`${service.id}.caption`)}</figcaption>
              </figure>
            </Fragment>
          ))}
        </div>
