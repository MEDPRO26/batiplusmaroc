      ...
      <div className="relative mx-auto max-w-[1280px]">
        <div className="grid items-end gap-9 border-b border-white/10 pb-10 md:grid-cols-[1fr_auto] md:pb-14">
          ...
          <div className="flex items-center gap-4 md:pb-2">
            <span className="text-[2.7rem] leading-none font-light tracking-[-0.06em] text-[#e7b63f]">04</span>
            <span className="max-w-24 text-[0.65rem] leading-4 font-bold tracking-[0.16em] text-[#8295a7] uppercase">
              {t("countLabel")}
            </span>
          </div>
        </div>
          ...
          <ol className="m-0 grid list-none gap-4 pt-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-0 lg:pt-14">
            {steps.map((step, index) => (
              ...
              >
                ...
                <span className="pointer-events-none absolute -right-2 top-12 text-[8rem] leading-none font-semibold tracking-[-0.09em] text-white/[0.035] transition-colors duration-300 group-hover:text-white/[0.055]" aria-hidden="true">
                  0{index + 1}
                </span>

                <div className="relative z-10 mt-auto pt-20">
                  <div className="mb-5 h-px w-10 bg-[#e7b63f] transition-all duration-300 group-hover:w-16" aria-hidden="true" />
                  <h3 className="process-card-title m-0 text-[1.45rem] font-semibold tracking-[-0.035em]">{t(`steps.${step}.title`)}</h3>
                  <p className="mt-3 mb-0 max-w-[255px] text-[0.95rem] leading-6 text-[#aab8c5]">{t(`steps.${step}.text`)}</p>
                </div>
              </li>
            ))}
          </ol>
